...
이 저장소(`MOVEAI_AXIS2`)는 **본선 제출본이 아니라 사전 프로토타입 · 구조 분석 ·
모듈 분해용 reference 저장소**입니다.

현재 저장소의 커밋은 전부 **2026-08-12 14:15 ~ 2026-08-13 02:51 (KST)** 사이에 작성되었습니다.
본선 공식 시작 시각인 **2026-08-13 09:00 이후에 작성된 커밋은 아직 없습니다.**
따라서 아래 표의 모든 항목은 **본선 시작 전에 이미 존재하던 상태**이며,
어느 항목도 "본선 당일 구현"으로 표기하지 않습니다.
...
| 영역 | 상태 |
|---|---|
| AXIS MOVE-AI MILP v7.1 최적화 엔진 | 별도 연구 산출물 (2026-08-10). 이 저장소는 결과 디렉터리만 읽음 |
| 화면 기획 (IA·와이어프레임) | 완료 — [`docs/planning/`](docs/planning) |
| MILP 결과 어댑터 · selector · 정합성 검증 | 완료 |
| Carrier Portal 5개 화면 | 완료 |
| KORAIL Control Tower 4개 화면 | 완료 |
| 운송물량 데이터 경로 (`transport_allocations`) | 완료 (selector · export · 화면) |
| 철도·트럭 운송 비교 | 완료 |
| 정적 배포 구성 (Vercel 정적 + 서버리스) | 구성 완료. **이 저장소는 Vercel 프로젝트에 연결되어 있지 않음** |
| MOVE-AI Copilot (Gemini) | 코드 연결 완료, **`GEMINI_API_KEY` 미주입** |
| 모듈 경계 분해 · 계약 문서화 | 완료 — [`docs/MODULES.md`](docs/MODULES.md) |
...
**2026-08-13 09:00 이후에 작성된 커밋만 본선 구현으로 기록합니다.**
그 시점 이전의 코드를 본선 당일 구현으로 서술하지 않으며,
커밋 날짜를 조작하거나 이력을 다시 쓰지 않습니다.
...
아래는 **배포 구성**입니다. 이 저장소 자체는 Vercel 프로젝트에 연결되어 있지 않습니다.
